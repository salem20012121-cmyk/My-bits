...
async def start(event):
    if event.sender_id != DEVELOPER_ID:
        try:
            ...
            return await event.respond(f"⚠️ اشترك أولاً في القناة لتتمكن من استخدام البوت:\n@{CHANNEL_USER}", 
                                     buttons=[Button.url("اضغط هنا للاشتراك", f"https://t.me/{CHANNEL_USER}")])
    
    await event.respond("🛡️ **أهلاً بك يا قائد في نظام الشد V12 (نسخة Render)**\nالقناة الحالية: @gbdbbd", buttons=MAIN_KB)
